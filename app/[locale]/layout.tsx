...
async function loadMessages(locale: "en" | "id" | "ja") {
  try {
    return (await import(`../internalization/${locale}.json`)).default as Record<string, string>;
  } catch {
    return (await import(`../internalization/en.json`)).default as Record<string, string>;
  }
}
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const ogLocale = localeTyped === "ja" ? "ja_JP" : localeTyped === "id" ? "id_ID" : "en_US";
